...
В примерах ниже строки одинаковой длины группируется вместе, да тех пор пока мы не встретим новую строку новой длины:
...
Для удобства `sort/2` позволяет нам передавать`:asc` или `:desc` в качестве второго парамтра:
...
Многие функции модуля Enum в Elixir принимают анонимные функции в качестве аргумента для работы с каждой итерацией переданного перечислимого.
...
Каждая версия функционально эквивалентна.
...
Теперь реализуем оператор захвата (&); захват каждой итерации списка чисел ([1,2,3]) и присвоение каждой итерации переменной &1, когда она передается через функцию отображения.
...
Это может быть улучшено, чтобы назначить предыдущую анонимную функцию с оператором захвата переменной и вызвать ее из функции `Enum.map/2`.
...
Сперва мы создаем именованную функцию и вызываем ее в анонимной функции, определенной в `Enum.map/2`.
